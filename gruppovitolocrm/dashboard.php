...
// Calcolo notifiche per dashboard
$new_orders_count = 0;
$new_reports_count = 0;
...
if (!$db_conn->connect_error) {
    if ($_SESSION['ruolo'] === 'admin') {
        $res = $db_conn->query("SELECT COUNT(*) AS c FROM ordini WHERE stato_ordine = 'Inviato' AND letto_admin = 0");
        if ($res) { $new_orders_count = (int)$res->fetch_assoc()['c']; $res->free(); }

        $res = $db_conn->query("SELECT COUNT(*) AS c FROM segnalazioni WHERE stato = 'Inviata' AND letto_admin = 0");
        if ($res) { $new_reports_count = (int)$res->fetch_assoc()['c']; $res->free(); }
    } else {
        $uid = $_SESSION['user_id'] ?? 0;
        $stmt = $db_conn->prepare("SELECT COUNT(*) AS c FROM ordini_chat oc JOIN ordini o ON oc.id_ordine = o.id_ordine WHERE oc.messaggio_admin IS NOT NULL AND oc.letto_da_utente = 0 AND o.id_utente_richiedente = ?");
        if ($stmt) {
        ...
        $stmt = $db_conn->prepare("SELECT COUNT(*) AS c FROM segnalazioni_chat sc JOIN segnalazioni s ON sc.id_segnalazione = s.id_segnalazione WHERE sc.messaggio_admin IS NOT NULL AND sc.letto_da_utente = 0 AND s.id_utente_segnalante = ?");
        if ($stmt) {
            $stmt->bind_param('i', $uid);
            $stmt->execute();
            $r = $stmt->get_result();
